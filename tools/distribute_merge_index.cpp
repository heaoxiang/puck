...
struct IndexFileStruct {
    ...
    void open(std::string& bs_base_path) {
        ...
        if (conf.whether_filter) {
            ...
            LOG(NOTICE)<<filter_data_file_name;
...
int main(int argc, char** argv) {
    ...
    LOG(NOTICE) << "start to process stdin lines";
    ...
    while (1){
        ...
        LOG(NOTICE)<<info.bs_id<<" "<<lastbsidstr;
        auto& bs_id = info.bs_id;
        if (bs_id != lastbsidstr) {
            if (lastbsidstr != "") {
                file_struct.close();

                // 上传文件目录包括fastsame和gnoimiindex
            ...
            // 创新新的fp-gnoimi文件对象, fp-fastsame文件对象
    ...
    LOG(NOTICE) << "finish to process stdin lines";
    // 上一个bs的索引merge完成，上传至hadoop输出目录
    if (lastbsidstr != "") {
        file_struct.close();
        LOG(NOTICE) << "finish to process stdin lines";
        ...
        if (system(ss.str().c_str()) != 0) {
            LOG(ERROR) << "gnoimi index file upload failed !";
            incrcounter("upload_fail");
            return -1;
        }
    }

    LOG(NOTICE) << "finish to process stdin lines with upload indexs";
